...
{
    // ─── GM Config Topbar buttons ────────────────────────────────────────────

    private void GmConfig_OpenCraftConfig()   => OpenCraftConfig_Click(null!, null!);
    private void GmConfig_SaveCraftConfig()   => SaveCraftConfig_Click(null!, null!);
    private void GmConfig_OpenAccessConfig()  => OpenAccessConfig_Click(null!, null!);
    private void GmConfig_SaveAccessConfig()  => SaveAccessConfig_Click(null!, null!);
...
// ─── State ──────────────────────────────────────────────────────────────
    ...
    private void SetGmConfigStatus(string text) => GmConfigStatus.Text = text;
    ...
    private void OpenCraftConfig_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show($"Не удалось открыть GM_CRAFTTABLE_CONFIG.\n\n{ex.Message}",
    ...
    private void SaveCraftConfig_Click(object sender, RoutedEventArgs e)
    {
        ...
        JsonService.Save(path, _craftConfig);
        SetGmConfigStatus($"GM_CRAFTTABLE_CONFIG сохранён: {path}");
    }
    ...
    private void OpenAccessConfig_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            SetGmConfigStatus($"GM_ACCESS_CONFIG загружен. Столов: {_accessConfig.CRAFT_TABLES.Count}");
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Не удалось открыть GM_ACCESS_CONFIG.\n\n{ex.Message}",
                "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
    ...
    private void SaveAccessConfig_Click(object sender, RoutedEventArgs e)
    {
        ...
        JsonService.Save(path, _accessConfig);
        SetGmConfigStatus($"GM_ACCESS_CONFIG сохранён: {path}");
    }
    ...
    {
        if (_craftConfig == null) return;
        if (CategoriesGrid.SelectedItem is not CategoryItem item) return;
    ...
    {
        if (_craftConfig == null || _craftConfig.CATEGORY_LIST.Count == 0) return 1;
    ...
    {
        if (_accessConfig == null) return;
        if (CraftTablesGrid.SelectedItem is not CraftTableAccess table) return;
    ...
    {
        if (_accessConfig == null || _accessConfig.CRAFT_TABLES.Count == 0) return 1;
    ...
    private void AddAdminId_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (_accessConfig.ADMIN_IDS.Contains(id))
        {
            SetGmConfigStatus($"ID уже есть в списке: {id}");
            return;
        }
        ...
        AdminIdText.Clear();
        RefreshAdminList();
        SetGmConfigStatus($"Добавлен админ: {id}");
    }

    private void DeleteAdminId_Click(object sender, RoutedEventArgs e)
    {
        if (_accessConfig == null) return;
        if (AdminIdsList.SelectedItem is not string id) return;

        _accessConfig.ADMIN_IDS.Remove(id);
        RefreshAdminList();
        SetGmConfigStatus($"Удалён админ: {id}");
    }
